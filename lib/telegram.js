...
// Edit an existing message (replaces text + removes keyboard)
async function editMessage(chatId, messageId, text) {
  const url  = getTelegramUrl("editMessageText");
  const body = { chat_id: chatId, message_id: messageId, text, parse_mode: "MarkdownV2" };
  const res    = await fetch(url, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(body) });
  const result = await res.json();
  if (!result.ok) {
    const plain = text.replace(/[\\*_`[\]()~>#+\-=|{}.!]/g, "");
...
module.exports = { sendMessage, sendDocument, sendAiReply, escMd, mdToTelegramMd, answerCallback, editMessage };